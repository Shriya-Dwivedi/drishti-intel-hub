...
def generate_answer_from_full_text(query: str, document_text: str, document_title: str) -> str:
    ...
    try:
        return _ollama_generate(prompt, num_predict=100, num_ctx=2048, temperature=0)
    except Exception:
        return (
            "Unable to generate an answer from the document at this time. "
            "Please review the source document below."
        )
...
def generate_rag_answer(query_text: str, matches: List[Dict[str, Any]]) -> str:
    ...
    try:
        return _ollama_generate(prompt, num_predict=80, temperature=0)
    except Exception:
        return (
...
def generate_summary(text: str, max_points: int = 4) -> str:
    ...
    try:
        summary = _ollama_generate(prompt, num_predict=120)
        if summary:
            return summary
    except Exception:
        pass
...
try:
    requests.post(
        ...
        json={
            ...
            "prompt": "Hello",
            "stream": False,
            "options": {"num_predict": 1, "num_ctx": 768},
        },
        timeout=120,
    )
