...
import { api, type Edge, type Entity, type DocumentRecord } from "@/services/api";
...
function EntitiesPage() {
  ...
  useEffect(() => {
    const filter = selectedDoc === "all" ? undefined : selectedDoc;
    api.getEntities(filter).then((d) => { setData(d); setActive(d.entities[0] ?? null); });
  }, [selectedDoc]);
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8 space-y-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            className="text-sm border border-border bg-card rounded-sm px-3 py-1.5"
          >
        ...
        <div className="grid lg:grid-cols-[1.6fr_1fr] gap-6">
          <div className="bg-card border border-border rounded-sm p-2 overflow-auto">
            {data.entities.length === 0 ? (
              <div className="p-10 text-center text-muted-foreground text-sm">No entities found for this selection.</div>
            ) : (
